declare
    ...
    @POSSyncStatus BIGINT =@tempPOSSyncStatus,
    @GroupID bigINT =@tempGroupID,
    @ItemListID bigint =@tempItemListID,
...
Begin set @Department =''                          
    ...
    DECLARE @ItemPriceSchedule TABLE                            
        (                            
          ...
          DECLARE @ItemListItem TABLE                            
        (                            
          ItemID BIGINT ,                            
          ItemListID BIGINT                           
                   
        )                    
            if(@ItemListID is not null )             
   begin                
    insert into @ItemListItem            
    select ItemID,ItemListID             
    from ItemListItem where ItemListID= @ItemListID            
                end             
    INSERT  INTO @Store                            
            SELECT  CompanyID ,                
                    StoreLocationID ,                            
...
WHERE   StoreLocationID IN (                            
            ...
            BEGIN                            
                INSERT  INTO @ItemList        
                        ...
                        WHERE   EXISTS ( SELECT 1                            
                                         FROM   ItemPriceGroup( NOLOCK ) ig                                                               WHERE  i.ItemID = ig.ItemID                            
  ...
  JOIN @Store StoreLocation ON StoreLocation.StoreLocationID = SLI.StoreLocationID                            
      ...
      AND Item.ItemID not in (Select ItemID from @ItemListItem)                            
 ...
 AND ( ROUND(SLI.InventoryValuePrice, 2) <= @InventoryValuePriceEnd                            
  ...
  AND ( ROUND(SLI.ProfitMargin, 2) >= @ProfitMarginStart                            
                          OR @ProfitMarginStart IS NULL                            
                        )                            
                    AND ( ROUND(SLI.ProfitMargin, 2) <= @ProfitMarginEnd                            
